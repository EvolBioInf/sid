#!/usr/bin/env python3
"""This module/script searches the Ensembl Core database for Exons that surround given genome sites.

The use case is the following: given a list of genome sites, e.g. SNPs, test for each of them if it
lies inside the exon of a gene. If it does, informations regarding the exon and gene is printed, see
...
columns = ','.join(['site.chrom', 'site.pos',
                   'gene.stable_id', 'gene.biotype',
                   'exon.exon_id','exon.seq_region_start','exon.seq_region_end','exon.seq_region_strand',
...
WHERE exon.seq_region_start <= site.pos
    ...
    AND (exon.seq_region_strand != 1 OR 
        (site.pos >= first.seq_region_start + tr.seq_start - 1 AND site.pos <= last.seq_region_start + tr.seq_end -1))
    AND (exon.seq_region_strand != -1 OR 
        (site.pos >= last.seq_region_end - tr.seq_end + 1 AND site.pos <= first.seq_region_end - tr.seq_start + 1))
